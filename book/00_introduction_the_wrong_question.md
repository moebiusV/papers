...
On April 3, Andrej Karpathy — co-founder of OpenAI and former Director of AI at Tesla — posted a personal knowledge system he'd been using for his own research.[^3] It is a three-folder markdown architecture. Raw sources are stored append-only and unchanged — the scroll that doesn't change (Chapter 2). An LLM compiles them into a structured, interlinked wiki with backlinks and cross-references — multi-level structural encoding (Chapter 3). Periodic "lint passes" scan the wiki for contradictions, gaps, and stale claims — adversarial verification (Chapter 4). No vector database. No retrieval-augmented generation. Just plain files in folders, organized by structure rather than embedding similarity. Karpathy had independently reinvented three of the six layers described in this book as a personal research tool.

Three days later, Milla Jovovich released MemPalace, an open-source memory framework built on the ancient method of loci.[^4] Instead of dumping conversations into a flat vector store and hoping semantic search finds the right thing, MemPalace organizes information into a navigable spatial hierarchy — wings, rooms, halls, drawers — the way the Greeks organized human memory two thousand years ago: by place, not by keyword. All classification runs on deterministic pattern matching, not the LLM — deterministic computation handling what deterministic computation handles best (Chapter 1). The spatial hierarchy itself is structural encoding that makes retrieval self-organizing rather than search-dependent (Chapter 3). Her benchmarks showed that the palace hierarchy alone improved retrieval by 34% over flat vector search — not better embeddings or reranking, just better organization.

Neither had read this book. Neither had consulted each other. They converged on pieces of the same architecture for the same reason every civilization in Chapter 4 converged on it: because it's the only architecture that works for intelligence operating in a world where storage is lossy and retrieval is unreliable. The convergence is the proof.
...
[^3]: Andrej Karpathy, "LLM Knowledge Bases," GitHub Gist, April 3, 2026. Karpathy described a system he had been running for months: raw source material in one folder (PDFs, web clips, notes), a compiled wiki in another (LLM-authored summaries with backlinks), and a schema file defining the organizational rules. At the time of posting, a single research topic had grown to roughly 100 articles and 400,000 words — longer than most PhD dissertations — without Karpathy writing a single word of it directly.

[^4]: Milla Jovovich and Ben Sigman, MemPalace, open-source release April 6, 2026. Jovovich had been using AI intensively since late 2025 and hit the same problem this book opens with: the model can't remember its own conversations. MemPalace mines conversations locally and organizes them into a spatial hierarchy. Within two days of release it had 23,000 GitHub stars. It is the highest-scoring free AI memory framework on the LongMemEval benchmark.
